...
<div class="card"><div class="table-wrapper"><table class="table">
    ...
    <tr>
        <td class="fw-600">{{ $k->frekuensi_kelas }}</td>
        <td>{{ $k->mataKuliah?->nama_mk }}</td>
        <td class="fs-12">{{ $k->jadwal_praktikum ?? '—' }}</td>
    ...
    @empty<tr><td colspan="7"><div class="empty-state"><p>Belum ada kelas praktikum.</p></div></td></tr>
    @endforelse
    </tbody>
</table></div></div>
<div id="modalTambah" class="modal-overlay"><div class="modal">
    ...
    <div class="grid grid-2">
        ...
        <div class="form-group"><label class="form-label required">Kelas / Frekuensi</label><input name="frekuensi_kelas" class="form-control" required placeholder="cth: Kelas A"></div>
        <div class="form-group"><label class="form-label">Jadwal</label><input name="jadwal_praktikum" class="form-control" placeholder="cth: Senin, 08:00–10:00"></div>
        ...
        <div class="form-group"><label class="form-label">Asisten</label><select name="asisten_id" class="form-select"><option value="">Pilih...</option>@foreach($asistenAll as $a)<option value="{{ $a->id }}">{{ $a->nama_asisten }}</option>@endforeach</select></div>
    </div>
