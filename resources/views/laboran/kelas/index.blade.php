...
<div class="page-toolbar" style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
    ...
    <button class="btn btn-danger" data-modal-open="modalHapusSemuaKelas"
...
{{-- Modal Hapus Semua Kelas --}}
<div id="modalHapusSemuaKelas" class="modal-overlay"><div class="modal" style="max-width:440px;">
    <div class="modal-header" style="background:#FEF2F2;border-bottom:1px solid #FECACA;">
        <span class="modal-title" style="color:#B91C1C;">⚠ Hapus Semua Kelas Praktikum</span>
        <button data-modal-close="modalHapusSemuaKelas" class="modal-close">✕</button>
    </div>
    <div class="modal-body">
        <p style="font-size:14px;color:#374151;margin:0 0 12px;">Tindakan ini akan menghapus <strong>seluruh kelas praktikum</strong> beserta semua data terkait:</p>
        <ul style="font-size:13px;color:#6B7280;margin:0 0 16px;padding-left:20px;line-height:1.8;">
            <li>Semua presensi mahasiswa</li>
        ...
        <div style="background:#FEF2F2;border:1px solid #FECACA;border-radius:6px;padding:10px 14px;margin-bottom:16px;font-size:13px;color:#B91C1C;">
            <strong>Tindakan ini tidak dapat dibatalkan.</strong> Data mahasiswa, dosen, dan asisten tidak ikut terhapus.
        </div>
        <p style="font-size:13px;color:#374151;margin:0 0 8px;">Ketik <strong>HAPUS SEMUA</strong> untuk konfirmasi:</p>
        <input type="text" id="konfirmasiHapusKelas" class="form-control" placeholder="HAPUS SEMUA" autocomplete="off">
    </div>
    <div style="display:flex;gap:8px;justify-content:flex-end;padding:16px;">
        <button type="button" data-modal-close="modalHapusSemuaKelas" class="btn btn-outline">Batal</button>
        <form method="POST" action="{{ route('laboran.kelas.hapus-semua') }}" id="formHapusSemuaKelas">
            @csrf @method('DELETE')
            <button type="submit" id="btnHapusSemuaKelas" class="btn btn-danger" disabled>Hapus Semua</button>
        </form>
    </div>
</div></div>
@push('scripts')
<script>
(function () {
    const input = document.getElementById('konfirmasiHapusKelas');
    const btn   = document.getElementById('btnHapusSemuaKelas');
    const form  = document.getElementById('formHapusSemuaKelas');
    if (!input) return;
    ...
    document.getElementById('modalHapusSemuaKelas')?.addEventListener('modal-close', () => {
